\begin{tikzpicture}[
    ...
    % Inputs
    \node[draw, rounded corners, fill=subtitle, minimum width=2.5cm, minimum height=0.7cm, above=of split] (input) {Sorted eigenvalues $\sigma$, rel. error tolerance $\epsilon$};
    ...
    \draw[arrow] (partition) -- (split); % recursion
    ...
    \draw[arrow] (partition_tails) -- (split); % recursion
    ...
    \node[draw, rounded corners, fill=white, font=\footnotesize, right=of output, yshift=0.5cm] (legend) {
        \begin{tabular}{ll}
            \textbf{Dotted rectangle}    & Recursion \\
            \textbf{Rounded, opaque box} & Input     \\
