\begin{tikzpicture}
    ...
    \draw[-{Latex[length=2mm, width=1.5mm]}] (O) -- (Rnew_tip) node[below left=1pt and 1pt] {$\mathbf{r}_{new}$};
    \draw[-{Latex[length=2mm, width=1.5mm]}] (Rnew_tip) -- (R0_tip) node[midway, above=.5pt] {$A\mathbf{c}$};
    \draw[-{Latex[length=2mm, width=1.5mm]}] (O) -- (R0_tip) node[above right=.5pt and .5pt] {$\mathbf{r}_{0}$};
