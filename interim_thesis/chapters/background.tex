...
\end{align*}
where $\kappa = \lambda_{\text{max}}/\lambda_{\text{min}}$ is the condition number of (the symmetric matrix) $A$. To sum up
\begin{theorem}
...
\[
  ||e_N||_A = ||\mathbf{e}_0||_A \max_{\lambda \in \sigma(A)} |\mathbf{r}_{\textrm{test}}(\lambda)| = 0,
\]
...
In the derivation of the convergence rate of the CG algorithm, we used the Chebyshev polynomial to bound the error. However, we can find an expression of the error provided the eigendecomposition of $A$ is available. Suppose $A = VDV^T$, then $r(A) = I - Aq(A) = V(I - Dq(D))V^T = Vr(D)V^T$. Also note that $\mathbf{e}_0 = x^* - \mathbf{u}_0 = A^{-1}b - \mathbf{u}_0 = A^{-1}\mathbf{r}_0$. As seen in \cref{eq:cg_convergence_rate}, the error of the $m^{\text{th}}$ iterate of the CG algorithm is given by
\begin{equation*}
...
which gives
\begin{equation}
  ||\mathbf{e}_m||_A^2 = \sum_{i=1}^n \frac{r_m(\lambda_i)^2}{\lambda_i} \rho_{0,i}^2.
...
\begin{enumerate}[label=\Roman*., ref=\textbf{ASM coarse space type \Roman*}]
