...
The first part of this chapter \cref{sec:cg_method} discusses the convergence rate of CG in \cref{sec:cg_convergence_rate} and how it is influenced by the distribution of the eigenvalues of the system matrix $A$ in \cref{sec:cg_eigenvalue_distribution}. The latter is important, since it shows that the condition number $\kappa(A)$ is not the only factor influencing the convergence rate of CG. Then we briefly review the PCG method in \cref{sec:cg_preconditioning} and how it can be used to improve the convergence rate of CG. Then, the second part of this chapter \cref{sec:schwarz_methods} concerns the Schwarz methods, which are a class of domain decomposition methods. These methods are used to solve PDEs on complex domains and can be used as preconditioners in PCG. In \cref{sec:schwarz_convergence}, we also see some conditioner number estimates that are classically used to estimate the convergence rate of PCG 
