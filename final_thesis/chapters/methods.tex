...
\[
    P_{\text{uniform}}(\kappa_l, \kappa_r) \gtrsim \frac{1}{\ln(4\kappa_r)} = \tilde{P}_{\text{uniform}}(\kappa_r).
\]
...
\begin{equation}
    \tilde{P}_{\text{uniform}}(\kappa_r) \lesssim P_{\text{uniform}}(\kappa_l, \kappa_r) \leq P(\kappa, \kappa_l, \kappa_r) \leq 1,
...
Now that we have established that the new bound is not absolutely sharper than the classical one, we can determine the conditions under which the new bound is sharper. We require $P\geq1$ to derive such conditions. This gives the following inequality
\[
