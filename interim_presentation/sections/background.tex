...
\begin{frame}[label=background,fragile]{Literature: CG Convergence for Non-uniform Spectra}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{.5\textwidth}
            ...
            \begin{itemize}
                ...
                \begin{equation*}
                    \|\mathbf{e}_m\|_A = \min_{r \in \mathcal{P}_{m-1}, r(0) = 1} \sum_{i=1}^{m}\frac{r(\lambda_i)^2}{\lambda_i}\rho_{0,i}^2
                \end{equation*}
    ...
    \end{columns}
    \only<6-8>{%
        CG iteration bound for relative error 
        \begin{equation*}
        ...
        \begin{column}{.5\textwidth}
            \only<1-3>{%
                \begin{itemize}
                    ...
                    \item<2-> Generalized framework: GDSW\footnote[frame]{\citeauthor{gdsw_coarse_space_Dohrmann2008} (\citeyear{gdsw_coarse_space_Dohrmann2008})}, RGDSW\footnote[frame]{\citeauthor{rgdsw_coarse_space_Dohrmann2017} (\citeyear{rgdsw_coarse_space_Dohrmann2017})}
                    \item<3-> Hybrid: AMS\footnote[frame]{\citeauthor{ams_coarse_space_comp_study_Alves2024} (\citeyear{ams_coarse_space_comp_study_Alves2024})}
                \end{itemize}
            }
            \only<4>{%
                \begin{figure}[H]
