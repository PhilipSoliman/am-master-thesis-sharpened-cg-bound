...
class PlotUpdate:
    ...
    def update_chebyshev_polys(self):
        ...
        ax.set_title(
            f"Chebpolys on clusters (#iter. bnd. improvement: {improvement:.2f}%)"
        )
...
C_product = np.exp(C_log_product)
C_spectrum_line = ax.plot(z, C_spectrum, label="$C_{" + f"{m_th}" + "}$", alpha=0.7)[0]
C_product_str = "$C_" + "C_".join("{" + f"{degree}" + "}" for degree in degrees) + "$"
C_product_line = ax.plot(z, C_product, label=f"{C_product_str}", alpha=0.7)[0]
improvement = (1 - sum(degrees) / m_th) * 100
ax.set_title(f"Chebpolys on clusters (#iter. bnd. improvement: {improvement:.2f}%)")
